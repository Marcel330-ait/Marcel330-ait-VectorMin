...
- 提供快速、标准、高清、超清质量预设。
- Adds a fidelity preset for more colors and sharper detail.
- 新增保真预设，用更多颜色保留更清晰的细节。
- Adjustable palette size, detail, path smoothing, curve fitting, and denoise filtering.
- 可调颜色数量、细节、平滑、曲线拟合和去噪。
- Includes Sobel and Canny-lite edge assist to protect contours during denoise and smoothing.
- 提供 Sobel 和 Canny-lite 边缘辅助，减少去噪和平滑导致的细节丢失。
...
## About 24-bit color / 关于 24-bit 真彩色

VectorMint does not convert every 24-bit bitmap color into its own SVG path, because that can create huge SVG files with millions of tiny regions. Instead, the fidelity preset raises the palette limit up to 32 colors and adds edge-assisted tracing for a more practical high-quality SVG output.

VectorMint 不会把 24-bit 位图里的每一种颜色都单独转成 SVG 路径，因为那会生成非常巨大的 SVG，甚至包含数百万个小区域。当前的保真预设会把颜色上限提高到 32 色，并加入边缘辅助描摹，在文件体积和输出质量之间取得更实用的平衡。
